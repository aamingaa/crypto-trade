...
class TimeBarBuilder(BaseDataProcessor):
    ...
    def __init__(self, time_interval: str):
        ...
        Args:
            time_interval: 时间间隔，如 '15min', '1h', '30min' 等
                          支持pandas resample的频率字符串
        """
    ...
    def process(self, data: pd.DataFrame) -> pd.DataFrame:
        """生成 Time Bars"""
        return self.build_time_bars(data, self.time_interval)
    
    def build_time_bars(self, trades: pd.DataFrame, time_interval: str) -> pd.DataFrame:
        """
        按照固定时间间隔生成 Time Bars
        ...
        # 添加bar_id (基于时间分组)
        df['bar_id'] = df['time'].dt.floor(time_interval).rank(method='dense').astype(int) - 1
